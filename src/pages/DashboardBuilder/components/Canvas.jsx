...
const Canvas = () => {
  ...
  const handleAddWidgetToRow = useCallback((rowId, widgetType = 'bar-chart') => {
    ...
    addWidget(widgetType, {
      ...
      index: widgetsCount,
      size: widgetsCount >= 2 ? 'small' : widgetsCount === 1 ? 'medium' : 'large'
    }, rowId);
      ...
      if (clientOffset) {
        ...
        if (targetRow) {
          ...
          if (getMaxWidgetsPerRow(targetRow.id) > 0) {
            ...
            if (item.type === WIDGET_TYPES.PROFESSIONAL_TABLE || item.type === WIDGET_TYPES.DATA_TABLE) {
              const newRowId = addRow();
              handleAddWidgetToRow(newRowId, item.type);
              return;
            }
            ...
            return (
              ...
              >
                {rowWidgets.map(widget => {
                  ...
                  const sizeClass = rowWidgets.length === 1 ? 'flex-1 w-full' :
                                   rowWidgets.length === 2 ? 'flex-1 w-1/2' :
                                   'flex-1 w-1/3';
                  ...
                  return (
                    <div 
                      ...
                      style={{
                        ...
                        flexBasis: rowWidgets.length === 1 ? 'calc(100% - 8px)' :
                                  rowWidgets.length === 2 ? 'calc(50% - 16px)' :
                                  'calc(33.333% - 16px)',
