...
export const BuilderProvider = ({ children }) => {
    ...
    (widgetId) => {
      ...
      const newWidget = {
        ...
        position: {
          ...widget.position,
          x:
            (widget.position.x + 1) % (gridConfig.cols - widget.position.w + 1),
  ...
  const reorderRows = useCallback(
    (sourceIndex, destinationIndex) => {
      ...
      const newWidgets = widgets.map((widget) => {
        const oldRowIndex = rows.findIndex(
          (r) => r.id === widget.position.rowId
        );
        const newRowIndex = newRows.findIndex(
          (r) => r.id === widget.position.rowId
        );
